...
* **Quick Chat 与 Workflows SOP 入口**：左侧栏的 **Quick Chat** 面向普通问答、解释、翻译和轻量咨询，创建普通 chat session 并自动将全局 `active_model_id` 重置为默认文本模型（优先 **Agnes 2.0 Flash**，否则首个可用 chat 模型），避免从生图模型切回新会话后仍走 image 路由；复杂任务编排由独立 **Workflows SOP** 导航承接，用户显式选择或设计工作流，不再依赖后端关键词猜测是否需要编排。折叠态与项目行内 `+` 保持轻入口语义。
