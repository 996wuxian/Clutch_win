...
- **Clutch 架构解法（现状与路线图）**：
  - **当前已实现**：支持基础的 **State 跨会话持久化与恢复**（`states/{run_id}.json`）、针对 CLI 引擎的 **Session 恢复机制**（`--resume` / Codex `exec resume`），以及 **长会话消息压缩（Compaction）**：当估算 token 接近上下文窗口阈值时自动折叠中间 assistant/tool 轮次，完整原文归档至 `runs/archive/{run_id}.jsonl`，并在 Chat 中展示压缩摘要卡片。
...
* **Observability Chat Feed**：支持对本地子进程 CLI（如 Claude Code CLI）敲击的所有终端命令及 stdout/stderr 输出进行行内展开卡片审计；聊天气泡中优雅地展示 Agent 专属标签、Boundary markers 和系统提示词 metadata，并在 AI 回复气泡下方显示本轮耗时。**Hybrid 回复的正文与图片均从 `outputEvents` 的 assistant 内容解析**，避免工作流上一节点气泡误显示下一节点生成的图片。Plain chat 支持 `client_message_id` 与乐观发送合并，避免切换 Agent 后重复「你好」等用户消息被去重或丢失。
* **Terminal Focus（Single Agent plain chat）**：当会话中存在已 Connect 的 CLI Agent（`*-cli` 路由键，如 Claude Code、OpenCode、Codex、Aider、CodeBuddy、Rivet、Ollama、Antigravity 等）时，主工作台提供 **Terminal Focus**，用于在同一个会话中展开 Terminal Orchestra，而不是切换到另一套任务模式。底部 `ChatInputBar` 保持为主输入，Active Agent 下拉仍可选择所有 Agent；Terminal Focus 内部嵌入 **OrchestratorBar**，专门用于 `@目标Agent` 自然语言或图语法派发、确认卡预览、handoff 文件写入与 Lane 间可视化（Orchestrator `@` 提及与已路由 CLI 显示名一致）。**OrchestratorBar 发送后，系统将任务文本注入目标 Lane 的 PTY（等待 TUI 就绪后自动提交 Enter）**；Ollama 以 `ollama run <model>` 启动交互会话。展开 Lane 超过 4 个时，终端区顶部显示分页圆点（每页 2×2）；**收起、翻页、关闭 Terminal Focus 或侧栏切换时 Lane PTY 保持连接，xterm 从缓冲 transcript 恢复并重绘，避免黑屏**。派发确认后 Overview **即时显示派发记录**（PTY 就绪前显示「正在打开终端…」，以 PTY 状态为准而非 lane `booting` 标签），时间戳按用户本地时区显示。Handoff「Send to Bar」填入图语法 `@目标 from @来源 @handoff.md`；Handoff 预览不再出现标题下空白卡片。用户可在 xterm Lane 内直接输入，也可在 Terminal Focus 内通过 OrchestratorBar 派发到其它 Lane，并继续使用底部 ChatInputBar 维持同一会话的普通对话。工作流 session 不展示 Terminal Focus。右侧监督面板 Tab 为 **Overview · Files · Changes · Terminal**（Overview 文本可选中复制）；Overview 展示派发记录（**含 CLI Session 恢复命令与 Copy**：Claude/CodeBuddy `claude --resume`、Codex `codex resume --last`、OpenCode `opencode -c` / `opencode -s`、Ollama/Antigravity 等按 Agent 类型生成，Clutch 内部分配的 UUID 不误用于不支持 resume 的 CLI）；Terminal 审计日志独立 Tab 保留。
